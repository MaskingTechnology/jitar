...
const lenientValidator = new Validator(false);
...
describe('Validator', () =>
{
    ...
    describe('Enum values', () =>
    {
        ...
        {
            const data = { enum: 'two' };
            const scheme = VALIDATION_SCHEMES.ENUM;
        ...
        {
            const data = { enum: 'four' };
            const scheme = VALIDATION_SCHEMES.ENUM;

            const result = strictValidator.validate(data, scheme);

            expect(result.valid).toBeFalsy();
            expect(result.errors).toContain(VALUES.MESSAGES.INVALID_ENUM_ITEM);
        });

        it('should reject a missing required enum value', () =>
        {
            const data = { enum: undefined };
            const scheme = VALIDATION_SCHEMES.ENUM;

            const result = strictValidator.validate(data, scheme);

            expect(result.valid).toBeFalsy();
            expect(result.errors).toContain(VALUES.MESSAGES.MISSING_ENUM);
        });
    });
    ...
    describe('Lenient mode', () =>
    {
        it('should accept extra fields', () =>
        {
            ...
            const result = lenientValidator.validate(data, scheme);
